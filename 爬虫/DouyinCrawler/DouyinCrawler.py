...
@app.get("/select_name/{nickname}")
async def read_user(nickname: str):
    try:
        res = {"result_data": getdata(nickname)}
    except:
        res = {"result_data":"there is nothing left."}
    return res
...
def getdata(username):
    ...
    for username,sec_id in username_and_sec_list:
        ...
        sc = sec_id
        res_tuple = (nickname, sc, video_list[0:10])
    return res_tuple


if __name__ == '__main__':
    uvicorn.run(app='DouyinCrawler:app', host="127.0.0.1", port=8000, reload=True, debug=True)
